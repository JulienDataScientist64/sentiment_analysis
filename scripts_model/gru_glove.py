...
# --------------------------------------------------
def parse_arguments():
    parser = argparse.ArgumentParser(description="Entraînement d'un modèle GRU bidirectionnel avec embeddings GloVe.")
    parser.add_argument("--data_path", type=str, required=True, help="Chemin vers le fichier CSV contenant les tweets.")
    parser.add_argument("--output_dir", type=str, required=True, help="Répertoire de sortie pour les modèles et rapports.")
    parser.add_argument("--embedding_dim", type=int, default=50, help="Dimension des embeddings GloVe.")
...
def main():
    ...
    print(f"[INFO] Chargement des données depuis {args.data_path}...")
    data = pd.read_csv(args.data_path)[['target', 'TweetText']]
    ...
    print("[INFO] Chargement des embeddings GloVe...")
    word_vectors = load('glove-twitter-50')
    ...
    for word, i in word_index.items():
        ...
        if word in word_vectors.key_to_index:
            ...
            embedding_matrix[i] = np.random.normal(scale=0.6, size=(args.embedding_dim,))

    # Construction du modèle GRU
    model = Sequential([
        Embedding(input_dim=num_words, output_dim=args.embedding_dim, weights=[embedding_matrix], input_length=args.max_len, trainable=False),
    ...
    model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10, batch_size=64, callbacks=[early_stopping, checkpoint_callback])
    end_time = time.time()
