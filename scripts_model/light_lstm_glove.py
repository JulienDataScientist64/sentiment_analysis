...
# --------------------------------------------------
def parse_arguments():
    parser = argparse.ArgumentParser(description="Light Bidirectional LSTM avec embeddings GloVe.")
...
def main():
    ...
    if device_name == '/device:GPU:0':
        ...
        tf.config.optimizer.set_jit(True)  # Active XLA pour accélérer
    else:
    ...
    def preprocess_texts(texts):
        ...
        return pad_sequences(seqs, maxlen=args.max_len)
    ...
    checkpoint_path = os.path.join(args.output_dir, "model_checkpoint_lstm_light.keras")
    ...
    report_path = os.path.join(args.output_dir, "classification_report_LSTM.txt")
    with open(report_path, 'w') as f:
        f.write(report)

    # Sauvegarde du modèle et du tokenizer
    model_path = os.path.join(args.output_dir, "final_trained_light_LSTM.h5")
